...
## 8. 联网搜索引擎自由切换与 Tavily 精准检索接入 (Tavily & Wikipedia Search Selector)
* **背景与痛点**：
  默认的维基百科检索偏向于词条百科概念，缺乏实战的、最新的、与特定开发语言（如 Rust/Go）底层实现相关的细节。且维基百科经常发生网络延迟和连接超时。
* **改进核心**：
  - **多引擎支持**：在 `types.ts` 和配置中引入 `searchProvider` 字段，允许在 `wikipedia` 和 `tavily` 之间进行自由切换。
  - **Tavily 接入**：当配置为 `tavily` 且指定 `tavilyApiKey` 时，系统将通过精准全网搜索引擎进行开发者文档检索，大幅提高课件中实战案例和最新技术的关联度。

---

## 9. 关卡快速预览与全课件静态离线预生成机制 (All Units Pre-Generation via `fc generate-all`)
* **背景与痛点**：
  在传统流程中，用户必须通关前一个单元才能解禁并看到下一个单元的讲义。对于想通览全局、把握整体课程图景的学习者而言，交互链路过于笨重。此外，在线生成课件有显著的网络延迟。
* **改进核心**：
  - **全局离线渲染**：新增了 `fc generate-all` 命令（在 `src/cli.ts` 中实现），支持一键并发离线预生成所有单元的讲义 Markdown 与代码骨架，并安全持久化，满足用户快速预览与无网/弱网离线学习的刚需。

---

## 10. 弹性跳过与复习面板机制 (Skip & Review Mechanism via `fc skip` and `fc review`)
* **背景与痛点**：
  传统严格的“通关即解锁”模式在遇到极其困难的单元时容易导致学习者沮丧和放弃。此外，缺乏一个专门的“账本”来记录和回顾那些曾跳过的难点。
* **改进核心**：
  - **主动跳过机制**：引入了 `fc skip` 指令，允许学习者主动跳过当前死磕的关卡。
  - **自动熔断引导**：引入了尝试次数监控，在 `fc submit` 失败达到 5 次时，系统会自动发出友好提示，引导学习者利用跳过功能前行，并保存进度方便后补。
  - **复习面板机制**：新建了 `fc review` 复习面板命令，展示所有曾跳过的单元列表，学习者随时可以返回重新发起挑战。

---

## 11. 实战驱动的中后期大中型 Project 生成 (Incremental Large-scale Project Integration)
* **背景与痛点**：
  纯基础的概念单元（如语法、基本数据结构）缺乏宏观项目视野，无法给学生提供像 CS61A *Ants* 或 *Scheme Interpreter* 那样能综合应用所学知识的实战快感。
* **改进核心**：
  - **自动注入 Project 关卡**：在大纲规划阶段，当单元总数 $\ge 4$ 时，自动在中后期插入一个 `type: "project"` 的关卡。
  - **特化 3-Pass 链路**：在大纲生成、课件撰写、Starter Code 生成等整个 3-Pass 链路中，Prompt 均针对 Project 类型进行了特化（生成 Project Spec 讲义、架构设计 Quiz，以及由简入繁的多阶段 Skeleton 模板与全局集成测试代码）。

---

## 12. 强关联趣味提示词重构与情绪价值诊断 (Cohesive Persona & Pedagogical Prompts Overhaul)
* **背景与痛点**：
  前期的生成文本冷冰冰、充满大模型的“翻译腔”和“机器味”；大纲和单元讲义缺乏故事主线，练习代码和测验也是干瘪无味的数学题或语法概念题。
* **改进核心**：
