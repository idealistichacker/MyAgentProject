...
* **项目描述**：一款基于 Multi-Agent 架构的计算机科学自学命令行客户端。系统通过对用户做交互式画像诊断，利用大模型自适应规划课程大纲；采用 3-Pass LLM 联网课件生成环输出定制化教案、练习与 CS61A 风格 Project 规格，并通过本地多语言编译执行沙盒（Runner Sandbox）及情绪共情力 AI TA 提供多阶段渐进式 Hints 批改服务。
* **核心工作与业绩（STAR 结构描述）**：
  ...
  2. **【抗限流调度 / 并发错峰】**：针对大批量生成任务时高频触发大模型服务商（SiliconFlow/OpenAI）速率超限（Rate Limit Exceeded）及 Socket 重置（`ECONNRESET`）问题，手写了基于 Promise 的 **Staggered Concurrency Limiter（错峰并发限制器）**，支持 `--concurrency` 与 `--stagger-ms` 参数，在默认稳健串行模式与高额度 API Key 的并发提速之间灵活切换。
  ...
  5. **【项目式课程产物建模】**：将 Project 从普通练习升级为一等结构化产物，设计 `ProjectSpec` schema 表达交付物、里程碑、文件清单、rubric 与扩展挑战，并在 `fc start` / `fc generate-all` 时渲染为 `PROJECT.md`，使生成内容从“单题练习”进化为可执行的大作业规格。
  6. **【失败驱动的学习路线自适应】**：基于 `AssessmentResult` 中的失败测试、Quiz 错题与错因分类，自动生成 `remediation` 微型补救单元并插入 `plan.json`；通过 `nextIfPassed` 路由机制让学习者通关补救后回到原单元，形成动态闭环的个性化学习路径。
  7. **【本地质量门禁 / 可观测性】**：实现 `fc audit` 课程质量审计器，覆盖计划路由、讲义长度、Quiz 答案一致性、练习测试覆盖、Project 里程碑/rubric 与 Remediation 回跳关系，输出质量分与 JSON 报告，提升生成内容的可验证性。
  8. **【AI TA 评估与归一化校验】**：编写了 Quiz 答题卡多格式归一化算法（支持字母、数字、右括号等格式兼容比对），融合“代码运行日志 + Quiz 答卷 + 源码”作为 LLM 上下文，提供场景化、多阶梯式的 Hints 调试启发，在通关和失败时提供差异化情绪支持。
...
> 1. 默认并发数为 1，将预生成退化为稳定的串行队列；同时通过 `--concurrency` 允许高额度 API Key 安全提速。
> 2. 在每个任务启动时，使用可配置的 `--stagger-ms` 启动间隔，避免短时间内的连续请求把套接字耗尽。
> 3. 对请求超时进行 `try-catch` 熔断，优先进行结构化修复与断点自适应重试，极大提升了分布式请求的抗震能力。”
