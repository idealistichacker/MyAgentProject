...
* **描述**：加载指定单元。若该单元为首次加载，将从网上爬取最新的标准规范（如 MDN、W3C 规范），并在后台重构 2-3 次自动生成专属课件与测试用例文件。生成结果会经过结构化解析、Zod 校验、内容长度/测试用例/Hint/入口函数等质量检查；如果模型输出格式不合格，系统会自动请求一次结构化修复后再落盘。
* **参数**：
