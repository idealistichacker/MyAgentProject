...
* **自适应补救**：如果同一单元连续失败到第 2 次，系统会根据失败测试、错题和诊断结果插入一个 `remediation` 补救单元。执行 `fc start` 学完补救单元并通过后，`fc next` 会自动带你回到原单元重新挑战。
...
### 13. `audit` — 课程质量审计
* **描述**：扫描当前 `plan.json` 中的讲义、Quiz、练习测试、Project 规格、补救单元路由和 fallback 标记，输出质量分与问题清单。
* **参数选项**：
  * `--json`：输出完整 JSON 报告，便于脚本或 CI 使用。
  * `--strict`：存在 warning 时也返回非 0 退出码。
* **示例**：
  ```powershell
  fc audit --strict
  ```
